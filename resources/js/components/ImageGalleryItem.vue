<template>
    <div class="md:w-1/2 xl:w-1/3 px-2 mb-8 relative">
        <input class="absolute h-0 w-0" ref="clipboardInput" type="text" :value="`${domain}/images/${image.id}`" readonly>
        <a :href="`${domain}/images/${image.id}`" class="block rounded border border-gray-light hover:border-green">
            <div class="absolute top-0 right-0 mr-4 z-50 mt-2 bg-black opacity-50 hover:opacity-100 p-2 rounded-full cursor-pointer" @click.prevent="copyLink">
